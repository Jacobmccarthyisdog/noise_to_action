...
def exclude_benchmark_portfolios(df, portfolio_col="Portfolio"):
    ...
    if df is None or portfolio_col not in df.columns:
...
def build_benchmark_history(prices, benchmark_map):
    ...
    for label, ticker in benchmark_map.items():
        if ticker in prices.columns:
            ...
            frame = frame.dropna(subset=["Benchmark Value"])
            frames.append(frame)
...
def get_complete_price_rows(prices, ticker_cols):
    """Return only dates with a complete market close for every tracked ticker."""
    cleaned = prices.copy()
    cleaned["Date"] = pd.to_datetime(cleaned["Date"])

    complete_prices = (
        cleaned.dropna(subset=ticker_cols)
        .sort_values("Date")
        .reset_index(drop=True)
    )

    if complete_prices.empty:
        raise ValueError("No complete market close row found for the configured ticker set.")
...
def build_datasets(portfolios, prices):
    ...
    complete_prices = get_complete_price_rows(prices, ticker_cols)

    price_long = complete_prices.melt(
        id_vars="Date",
    ...
    benchmark_history = build_benchmark_history(complete_prices, BENCHMARK_MAP)
